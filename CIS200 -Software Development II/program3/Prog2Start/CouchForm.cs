...
namespace Prog3Start
{
    public partial class CouchForm : Form
    {
        ...
        public Couch CreatedCouch {  get; set; }
        ...
        private void createButton_Click(object sender, EventArgs e)
        {
            if (this.ValidateChildren())
            {
                ...
                Cushion cushion = new Cushion(cusMatComboBox.Text,double.Parse(lengthTextBox.Text), double.Parse(widthTextBox.Text), double.Parse(heightTextBox.Text));
                CreatedCouch = new Couch(decimal.Parse(priceTextBox.Text),categoryTextBox.Text,double.Parse(weightTextBox.Text),int.Parse(seatsTextBox.Text),int.Parse(legsTextBox.Text),materialComboBox.Text,cushion);
                this.Close();
            }
        }
